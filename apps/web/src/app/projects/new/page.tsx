...
import type { Channel, ScriptBreakdown, BackgroundMusic } from '@/types';
            ...
            <div className="grid gap-6 sm:grid-cols-2">
                ...
                <div className="space-y-3">
                    ...
                    <RadioGroup
                        ...
                        onValueChange={(v) => setSettings({ channelId, format, type, videoResolution: v as '480p' | '720p' })}
                        className="flex gap-4"
                    >
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="480p" id="res480" />
                            <Label htmlFor="res480">480p</Label>
                        </div>
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="720p" id="res720" />
                            <Label htmlFor="res720">720p</Label>
                        </div>
                    </RadioGroup>
                </div>
...
function Step4SceneVideos() {
    const { scenes, updateScene, setStep, channelId, format, videoResolution, audioConfig } = useProjectStore();
    const [generatingIndex, setGeneratingIndex] = useState<number | null>(null);
    const [generatingAll, setGeneratingAll] = useState(false);
...
function Step5Final() {
    ...
    const [musicOption, setMusicOption] = useState("auto"); // Changed default to auto
    ...
    useEffect(() => {
        const fetchMusic = async () => {
            ...
            try {
                const res = await api.get<BackgroundMusic[]>('/audio/background-music');
                setMusicTracks(res);
            } catch (e) {
    ...
    const categorizedMusic = useMemo(() => {
        const groups: Record<string, BackgroundMusic[]> = {};
    ...
    const handleStitch = async () => {
        setIsStitching(true);
        // Stop any playing audio if needed
        if (audioRef.current) {
            audioRef.current.pause();
            audioRef.current = null;
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-1">
                <MusicSelector />
                {/* Column 2: Voiceover & Audio Configuration (Hidden per user request) */}
                {/* 
                <div className="space-y-4">
                    ...
                </div>
...
function MusicSelector() {
    const { 
        selectedMusicId, setMusicId, musicOption, setMusicOption 
    } = useProjectStore();
    ...
    const categorizedMusic = useMemo(() => {
        ...
        // Sort categories: "Cartoon" first, "Ghibli Cozy" second, then others alphabetically
        const sortedCategories = Object.keys(groups).sort((a, b) => {
            if (a === "Cartoon") return -1;
            if (b === "Cartoon") return 1;
            if (a === "Ghibli Cozy") return -1;
            if (b === "Ghibli Cozy") return 1;
            return a.localeCompare(b);
        });

        const sortedGroups: Record<string, BackgroundMusic[]> = {};
    ...
    return (
        <div className="space-y-4">
            <Label className="text-sm font-medium">Background Music Configuration</Label>
            
            <div className="flex gap-2 items-center">
                <div className="w-full max-w-[400px]">
                     <Select value={selectedMusicId || musicOption} onValueChange={(val) => {
                ...
                <Button
                    ...
                    disabled={!selectedMusicId && (musicOption === "auto" || musicOption === "none")}
